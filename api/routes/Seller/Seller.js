    ...
    if (!exists) {
        ...
        for (let i = 0; i < seller.product.length; i++) {
            if (productList[i].modelNumber == modelNo) {
                ...
                const originalProductQuantity = parseInt(
...
router.get("/getProductTokens", async (req, res) => {
    ...
    res.json((parseInt(balance,16))/1000000000000000000);
});
...
router.post("/sendProducts", async (req, res) => {
    ...
    for (let i = 0; i < sellerArray.length; i++) {
        ...
        if (productModelNo === sellerArray[i].modelNumber) {
            console.log(
                "seller bhai ke products before decrement:",
                sellerArray[i].quantity
            );
            sellerArray[i].quantity -= products;
            console.log(
                "seller bhai ke products after decrement:",
                sellerArray[i].quantity
            );
        }
    }
